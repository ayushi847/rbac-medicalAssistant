...
st.set_page_config(page_title="Health RBAC RAG Chatbot",layout="centered") 
    ...
    with tab2:
        ...
        if st.button("Signup"):
            ...
            if res.status_code==200:
                ...
                st.success(f"Signup successfull! You can login.")   
                st.rerun()
            else:
...
def chat_interface():
    ...
    if st.button("Send"):
        ...
        if res.status_code==200:
            ...
            if reply.get("answer")=="No relevant info found":
